#!/usr/bin/env python3
"""
YanHH3D Scraper → MonPlayer JSON (v5.3 - The Passive Sniffer)
✅ Giải quyết cốt lõi: Để trình duyệt tự click, tự gọi API, ta chỉ rình bắt link MP4 cuối cùng
✅ Không dùng route.fetch() làm hỏng ngữ cảnh nữa
✅ Dùng đúng Fingerprint của Chrome để vượt qua tường lửa (nếu nó based on TLS)
✅ Bỏ qua SV2/Embed đi lối because Thuyết Minh chỉ có play-fb-v8
"""
...
def get_stream_url(page: Page, ep_url: str):
    """
    🔥 CORE LOGIC V5.3: The Passive Sniffer
    Không gọi API bằng code. Để trình duyệt click, và rình bắt link MP4 cuối cùng.
    """
    try:
        ...
        # Tìm nút Thuyết Minh (chứa play-fb-v8)
        btn = page.query_selector("#list_sv a.btn3dsv[data-src*='play-fb-v8']")
        if not btn:
            btn = page.query_selector("#list_sv a.btn3dsv[data-src*='thuyetminh']")
        ...
        def catch_fb_mp4(response):
            nonlocal intercepted_url
            url = response.url
            # Bắt chính xác link MP4 của Facebook CDN
            if 'scontent' in url and '.mp4' in url and response.status == 200:
                intercepted_url = url

        # Bắt đầu rình mạng lưới
        page.on('response', catch_fb_mp4)
        
        # Click như một người thật
        btn.click()
        
        # Đợi Javascript của web xử lý API và ném link vào player
        try:
            page.wait_for_function("""() => {
                const video = document.querySelector('video');
                if (video && video.src && video.src.includes('scontent')) return true;
                const source = document.querySelector('video source');
                if (source && source.src && source.src.includes('scontent')) return true;
                return false;
            }""", timeout=8000)
        except Exception:
            # Dù không tìm thấy thẻ video, có thể JS vẫn gọi link MP4 rồi
            pass
            
        page.wait_for_timeout(1000) # Chờ thêm 1 giây cho chắc chắn
        page.remove_listener('response', catch_fb_mp4)
        
        # Nếu rình được link MP4
        if intercepted_url:
            return [{"url": intercepted_url, "type": "mp4", "label": "fb-cdn-browser"}]
            
        # Fallback: Trích xuất từ DOM nếu network fail
        dom_url = page.evaluate("""() => {
            const video = document.querySelector('video');
            if (video && video.src && video.src.includes('scontent')) return video.src;
            const source = document.querySelector('video source');
            if (source && source.src && source.src.includes('scontent')) return source.src;
            return null;
        }""")
        
        if dom_url:
            return [{"url": dom_url, "type": "mp4", "label": "fb-cdn-dom"}]
...
def scrape_movie(page: Page, movie_info: dict, movie_index: int, total_movies: int,
    ...
    for i in range(crawl_limit):
        ...
        if stream:
            ...
            stream_type = stream[0].get("type", "unknown")
            logger.info(f"  [{movie_index}/{total_movies}]    ✓ Tap {ep['name']} ({stream_type})")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="YanHH3D → MonPlayer Scraper v5.3 (Passive Sniffer)")
    ...
    logger.info(f"Starting YanHH3D Scraper (v5.3 - The Passive Sniffer)...")
